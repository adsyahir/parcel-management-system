...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
 ...
 <c:when test="${s.staffPosition.equals('staff')}">
    <div hidden="hidden">
	 ...
	 <c:when test="${s.staffPosition.equals('staff')}">
	     <div hidden="hidden">
	     <a href="ListStaffController?sid=${s.staffID}" class="w3-bar-item w3-button w3-padding w3-blue"><i class="fa fa-eye fa-fw"></i> Staff List</a>
	 	 </div>
	 	</c:when>
	 	<c:otherwise>
	 	<a href="ListStaffController?sid=${s.staffID}" class="w3-bar-item w3-button w3-padding w3-blue"><i class="fa fa-eye fa-fw"></i> Staff List</a>
  ...
  <div class="w3-container">
    ...
    <table class="w3-table w3-striped w3-bordered w3-border w3-hoverable w3-white">
		...
		<tr>
			...
			<td><c:out value="${staff.staffPosition}" /></td>
			<td><c:out value="${staff.staffEmail}" /></td>
			<td><c:out value="${staff.staffAddress}" /></td>
			<td><c:out value="${staff.adminID}" /></td>
     <td> <a href="DeleteStaffController?staffid=<c:out value="${staff.staffID}" />&sid=${s.staffID}"><button type="button" class="btn btn-outline-danger" >Delete</button></a></td>
 		</tr>
		</c:forEach> 
	</table>	
</div>
